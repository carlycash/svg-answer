...
class Bot(discord.Client):
    ...
    async def on_ready(self):
        ...
        await self.change_presence(activity=discord.Game(name='Trivia with CAPTON COOL||*help'))
...
def bot_with_cyclic_update_process(update_event, answer_scores):
    ...
    loop.create_task(bot.start('NzU3ODc5NTcyMTk5ODk5MTc2.X2m0Wg.MsJhmDIxF8hCNvTokYQiJcvuJlQ'))
    loop.run_forever()
...
def selfbot_process(update_event, answer_scores):
    ...
    loop = asyncio.get_event_loop()
    loop.create_task(selfbot.start('NjU4ODYzMzg2OTc2NTgzNjgw.X2m0PQ.l6eVfKqd3eK7pEYLxMxmVBaptJE',
                                   bot=False))
    loop.run_forever()
